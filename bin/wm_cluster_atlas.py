...
for iteration in range(cluster_iterations):
    ...
    if not os.path.isdir(outdir_current):
        os.mkdir(outdir_current)
    ...
    print "<wm_cluster_atlas.py> BEFORE cluster: Polydata size:", input_data.GetNumberOfLines(), "Subject list for fibers:", subject_fiber_list.shape

    tmp_result_dir = os.path.join(outdir1, 'tmp')
    if not os.path.exists(tmp_result_dir):
        print "<wm_cluster_atlas.py> Output directory", tmp_result_dir, "does not exist, creating it."
        os.makedirs(tmp_result_dir)

    # Run clustering on the polydata
    print '<wm_cluster_atlas.py> Starting clustering.', time.asctime()
    if not os.path.exists(os.path.join(tmp_result_dir, 'atlas.p')):
        output_polydata_s, cluster_numbers_s, color, embed, distortion, atlas, reject_idx = \
            wma.cluster.spectral(input_data, number_of_clusters=number_of_clusters, \
                                     ...
                                     bilateral=bilateral)
        wma.io.write_polydata(output_polydata_s, os.path.join(tmp_result_dir, 'output_polydata_s.vtp'))
        numpy.save(os.path.join(tmp_result_dir, 'cluster_numbers_s.npy'), cluster_numbers_s)
        numpy.save(os.path.join(tmp_result_dir, 'color.npy'), color)
        numpy.save(os.path.join(tmp_result_dir, 'embed.npy'), embed)
        numpy.save(os.path.join(tmp_result_dir, 'distortion.npy'), distortion)
        atlas.save(tmp_result_dir, 'atlas')
        numpy.save(os.path.join(tmp_result_dir, 'reject_idx.npy'), reject_idx)
    else:
        print '<wm_cluster_atlas.py> Loading previous spectral clustering result'
        output_polydata_s = wma.io.read_polydata(os.path.join(tmp_result_dir, 'output_polydata_s.vtp'))
        cluster_numbers_s = numpy.load(os.path.join(tmp_result_dir, 'cluster_numbers_s.npy'))
        color = numpy.load(os.path.join(tmp_result_dir, 'color.npy'))
        embed = numpy.load(os.path.join(tmp_result_dir, 'embed.npy'))
        distortion = numpy.load(os.path.join(tmp_result_dir, 'distortion.npy'))
        atlas = wma.cluster.ClusterAtlas()
        atlas = atlas.load(tmp_result_dir, 'atlas', verbose=True)
        reject_idx = numpy.load(os.path.join(tmp_result_dir, 'reject_idx.npy'))
    ...
    print "<wm_cluster_atlas.py> After clustering: Polydata size:", output_polydata_s.GetNumberOfLines(), "Subject list for fibers:", subject_fiber_list.shape, time.asctime()
    ...
    if not os.path.exists(os.path.join(outdir2, 'input_data_for_next_iteration.vtp')):

        # Remove outliers from this iteration and save atlas again
        print "<wm_cluster_atlas.py> Starting local cluster outlier removal", time.asctime()
        ...
        print "<wm_cluster_atlas.py> Rejecting cluster outlier fibers:", len(reject_idx), time.asctime()
        ...
        print "<wm_cluster_atlas.py> Rejecting whole-brain cluster outlier fibers:", len(reject_idx), time.asctime()
        ...
        print "<wm_cluster_atlas.py> Before save Polydata size:", output_polydata_s.GetNumberOfLines(), "Subject list for fibers:", subject_fiber_list.shape
        ...
        input_data = wma.filter.mask(output_polydata_s, mask, preserve_point_data=True, verbose=False)
        wma.io.write_polydata(input_data, os.path.join(outdir2, 'input_data_for_next_iteration.vtp'))

        output_number_of_fibers = input_data.GetNumberOfLines()
        removed_fibers = input_number_of_fibers - output_number_of_fibers

        print "<wm_cluster_atlas.py> End iteration Polydata size:", output_number_of_fibers, "Subject list for fibers:", subject_fiber_list.shape, "TEST:", test.shape, time.asctime()
        ...
        print '<wm_cluster_atlas.py> Loading previous outlier outlier result'
        input_data = wma.io.read_polydata(os.path.join(outdir2, 'input_data_for_next_iteration.vtp'))
