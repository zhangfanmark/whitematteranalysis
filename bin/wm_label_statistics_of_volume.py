...
parser.add_argument(
	...
	help='Directory of input labelmaps (e.g., freesurfer parcellation). The script will calculate label statistics of the input volume (as in LabelStatistics module in 3D Slicer) given each region of the input label map.')
parser.add_argument(
	'inputVolumeDirectory',
	help='Directory of input image volume (e.g., T1, T2 or B0 image). Note: Image volume and the corresponding labelmap must have same dimensions.')
parser.add_argument(
	'outputFile',
	help='A CVS file that contains the statistics.')
parser.add_argument(
	'Slicer',
	help='Path of 3D Slicer.')
parser.add_argument(
	'-r',  dest='region', type=int,
	help='A list of selected regions. If not given, all regions will be included.')
